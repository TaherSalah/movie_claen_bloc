...
class TvTrendingSeeMore extends StatelessWidget {
  const TvTrendingSeeMore({super.key});

  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => sl<MovieBloc>()..add(const GetTvTrendingMoviesEvent()),
        ...
        builder: (context, state) {
          switch (state.getTvTrendingMoviesState) {
            ...
            case RequestStates.loaded:
              return Scaffold(
                  appBar: AppBar(
                    title: const Text('All Tv Trending Movies'),
                  ),
                  body: GridView.builder(
                    padding: const EdgeInsets.symmetric(vertical: 15),
                    itemCount: state.getTvTrendingMovies.length,
                    gridDelegate:
                            ...
                            Column(
                              ...
                              children: [
                                ...
                                Padding(
                                  ...
                                  child: Text(
                                    state.getTvTrendingMovies[index].name,
                                    style: TextStyle(
                                        fontSize: 10.sp,
                                    ...
                                    children: [
                                      CircularPercentIndicator(
                                        ...
                                        percent: double.parse(
                                                '${state.getTvTrendingMovies[index].voteAverage}') /
                                            9,
                                        // here we're using the percentage to be in sync with the color of the text
                                        center: Text(
                                          ("${(state.getTvTrendingMovies[index].voteAverage / 9 * 100).round()}%"),
                                          style: TextStyle(
                                              fontWeight: FontWeight.bold,
                                              fontSize: 9.0.sp,
                                              color: Colors.black),
                                      ...
                                      Container(
                                        ...
                                        child: Text(
                                          state.getTvTrendingMovies[index]
                                              .firstAirDate
                                              .split('-')[0],
